...
async def _get_task_metadata(task_id: str) -> Optional[Dict[str, Any]]:
...
async def _set_task_metadata(task_id: str, metadata: Dict[str, Any]) -> None:
...
async def create_task(request: Dict[str, Any], background_tasks: BackgroundTasks):
    ...
    await _set_task_metadata(task_id, {
        "task_id": task_id,
        "thread_id": thread_id,
        "user_id": user_id,
        "tenant_id": tenant_id,
