...
# Global Redis client (initially None, will be created lazily)
...
async def get_redis_client() -> Redis:
    ...
    global redis_client
    if redis_client is None:
        # In production this should be created in FastAPI lifespan events.
        # For standalone/testing, create it lazily here.
        logger.warning(
...
async def create_proactive_task_to_queue(
    ...
    """Creates a proactive task and dispatches it to a Redis Stream for asynchronous processing."""
    task_id = str(uuid.uuid4())
    thread_id = str(uuid.uuid4())
    now = datetime.now(timezone.utc).isoformat()

    logger.info(
        "proactive_task.creating_and_queuing",
        task_id=task_id,
        task_description=task_description,
        priority=priority,
    )
    ...
    try:
        ...
        logger.info(
            "proactive_task.dispatched_to_redis_stream",
            task_id=task_id,
            stream="proactive_tasks_stream",
        )
        return {"status": "dispatched_to_queue", "task_id": task_id}
    except Exception as exc:
        error_trace = traceback.format_exc()
        logger.error(
            "proactive_task.dispatch_failed_redis",
            task_id=task_id,
            error=str(exc),
...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    # Initialize CogneeMemory for the worker tools
    ...
    # Register tools (ensure they use the same _cognee_memory instance)
    _registry.register(BrowserTool())
    _registry.register(PythonREPLTool())
