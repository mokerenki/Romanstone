...
# aioredis removed – replaced with redis.asyncio
...
class HTTPProbe(BaseProbe):
    ...
    async def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not status_ok:
                result["error_details"] = response.text[:200]  # Capture first 200 chars
            return result
            ...
            return {
                ...
                "probe_name": self.name,
                "url": url,
                "message": f"HTTP {method} to {url} timed out after {timeout}s",
            }
            ...
            return {
                ...
                "probe_name": self.name,
                "url": url,
                "message": f"HTTP {method} to {url} failed: {str(e)}",
            }
            ...
            return {
                ...
                "probe_name": self.name,
                "url": url,
                "message": f"Unexpected error during HTTP probe: {str(e)}",
            }
        ...
        try:
            if check_type == "exists":
                ...
                if not os.path.isfile(path):
                    return {
                        "status": "critical",
                        "message": f"Path is not a file for size check: {path}",
                        "probe_name": self.name,
                    }
                ...
                return {
                    ...
                    "message": f"File size: {current_size} bytes",
                    "probe_name": self.name,
                }
                ...
                if not os.path.isfile(path):
                    return {
                        "status": "critical",
                        "message": f"Path is not a file for modified_age check: {path}",
                        "probe_name": self.name,
                    }
                if threshold is None:
                    return {
                        "status": "error",
                        "message": "Threshold (seconds) not configured for modified_age check",
                        "probe_name": self.name,
                    }
                ...
                return {
                    ...
                    "message": f"File last modified {round(age_seconds, 2)} seconds ago",
                    "probe_name": self.name,
                }
                ...
                if not os.path.isdir(path):
                    return {
                        "status": "critical",
                        "message": f"Path is not a directory for new_files check: {path}",
                        "probe_name": self.name,
                    }
                ...
                new_files = {
                    f for f in current_files if f not in last_state or current_files[f] > last_state.get(f, 0)
                }
                ...
                if len(new_files) > 0:
                    return {
                        ...
                        "message": f"{len(new_files)} new or modified files detected in {path}",
                        "probe_name": self.name,
                    }
                else:
                    return {
                        ...
                        "path": path,
                        "new_files_count": 0,
                        "message": f"No new or modified files in {path}",
                        "probe_name": self.name,
                    }
            ...
            return {
                ...
                "probe_name": self.name,
                "path": path,
                "message": f"Unexpected error during File probe: {str(e)}",
            }
...
class QueueProbe(BaseProbe):
    """Checks Redis Stream/List depth using redis.asyncio."""
    async def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if depth >= threshold_critical:
                status = "critical"
                message = (
                    f"Queue `{queue_name}` depth ({depth}) exceeds critical threshold "
                    f"({threshold_critical})"
                )
            elif depth >= threshold_warn:
                status = "warn"
                message = (
                    f"Queue `{queue_name}` depth ({depth}) exceeds warning threshold "
                    f"({threshold_warn})"
                )
            ...
            return {
                ...
                "probe_name": self.name,
                "queue_name": queue_name,
                "message": f"Redis error during queue probe: {str(e)}",
            }
        except Exception as e:
            return {
                "status": "error",
                "probe_name": self.name,
                "queue_name": queue_name,
                "message": f"Unexpected error during Queue probe: {str(e)}",
            }
